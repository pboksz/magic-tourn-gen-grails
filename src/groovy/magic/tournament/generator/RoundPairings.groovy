...
class RoundPairings {
   ...
   private ArrayList<PlayerInfo> sortByInitialSeed() {
      def listOfPlayers = PlayerPool.getListOfPlayers()
...
//      printRankings()
   ...
   def trySettingLowestBye(ArrayList<PlayerInfo> sorted, int last) {
      def byeIsNotSet = true
      if (PlayerPool.numPlayers == 3) {
    ...
    */
   def handle3PlayerByes(ArrayList<PlayerInfo> sorted, int last) {
      def byeIsNotSet = true
      ...
      if (tourn.round == tourn.maxRound) {
         for (i in 0..sorted.size()) {
            def player = sorted.get(i)
